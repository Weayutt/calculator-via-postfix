# based on template given by Larissa Munishkina
# Author: Wyatt Avilla
# Date: 11/15/22
# File: Interacts with calculator.py via a GUI

from tkinter import *
from calculator import calculate
from calculator import format_okay
import time
...
def clickButton(entrybox, value):
    if value == "=":
        entry = entrybox.get()
        if len(entry) > 1:
            if format_okay(entry):
                display = (calculate(entry))
                entrybox.delete(0,"end")
                entrybox.insert(0,str(display))
                # entrybox.after(2000, entrybox.delete(0,"end"))
            else:
